...
// Almost there - has a bug for input above 512 bytes
#[derive(Debug)]
pub struct Sha2 {
    h: [u32; 8],
    k: [u32; 64]
...
impl Sha2 {
    ...
    pub fn algo(self, filename: &String) {

	//Pre-processing (Padding):
	match self.preprocess(filename) {
	    Ok(s) => {
		println!("{:?}", s);
	    }
	    Err(_) => {
    ...
    fn preprocess(mut self, filepath: &String) -> io::Result<String> {
	// Create path to file
